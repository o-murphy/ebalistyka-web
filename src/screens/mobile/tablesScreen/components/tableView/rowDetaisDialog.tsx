...
const RowDetailsDialog: React.FC<RowDetailsProps> = ({ row, visible, setVisible }) => {
    ...
    const data = [
        ...
        row.slantHeight.In(preferredUnits.drop).toFixed(1),
        row.dropAngle.In(preferredUnits.adjustment).toFixed(2),
        row.windage.In(preferredUnits.drop).toFixed(1),
        row.windageAngle.In(preferredUnits.adjustment).toFixed(2),
        row.mach.toFixed(2),
        row.drag.toFixed(3),
