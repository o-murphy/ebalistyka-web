...
const saveCurrentConditions = async (currentConditions) => {
  try {
...
const loadUserData = async () => {
  try {
    ...
    return [profileValue, conditionsValue]
  } catch (error) {
...
const useUserData = async (setProfileProperties, setCurrentConditions, setIsLoaded) => {

  const [profileValue, conditionsValue] = await loadUserData()
  ...
  if (conditionsValue != null && conditionsValue !== 'null') {
    setCurrentConditions({...defaultConditions, ...JSON.parse(conditionsValue)});
  }
...
export const ProfileProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    useUserData(setProfileProperties, setCurrentConditions, setIsLoaded); // Load data on mount
  }, []);
  ...
  useEffect(() => {
    if (isLoaded) { // Only save if data has been loaded
      saveCurrentConditions(currentConditions); // Save current conditions whenever they change
    }
  ...
  const updateCurrentConditions = (props: Partial<ProfileProps>) => {
